...
-- Daily revenue with 7-day moving average (useful for short-term trend spotting)
SELECT
    d.full_date,
...
-- Which sellers have the worst review scores? (join to fact_sales)
SELECT
    s.seller_id,
    COUNT(*) AS reviews,
    ROUND(AVG(r.review_score), 2) AS avg_score,
...
-- Revenue generated by leads (requires hopping to fact_sales via dim_seller)
SELECT
    l.origin,
...
WHERE fl.is_closed
  AND fl.seller_key != -1   -- exclude unknown sellers
GROUP BY l.origin
...
-- -------------------------------------------------------------------------
-- Q8: Demonstrate SCD Type 2 accuracy: revenue by product category uses
--     the category version at the time of sale
-- -------------------------------------------------------------------------
-- This query joins fact_sales to dim_product using product_key, so each
-- sale naturally links to the version that was current at purchase time.
SELECT
...
-- Compare: if a product changed category from "A" to "B" on 2018-06-01,
-- sales before that date join to the old version (category A), and sales
-- after join to the new version (category B). Revenue is not misattributed.
